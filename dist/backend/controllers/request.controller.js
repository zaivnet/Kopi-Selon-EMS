...
export const createRequest = async (req, res) => {
    try {
        ...
        const { type, permissionType, startDate, endDate, targetShiftId, targetEmployeeId, reason, attachmentUrl, isDraft, } = req.body;
...
export const respondPeerSwap = async (req, res) => {
    try {
        const { id } = req.params;
...
export const getEligibleSwapPeers = async (req, res) => {
    try {
        const userId = req.user?.id;
        const currentEmployee = await prisma.employee.findFirst({
            where: { userId, deletedAt: null },
        ...
        const eligiblePeers = activeEmployees
            ...
            if (permissionEmployeeIds.has(emp.id))
                return false;
            ...
            if (activeSwapEmployeeIds.has(emp.id))
                return false;
            return true;
        })
            .map((emp) => {
